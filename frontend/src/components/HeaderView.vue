<template>
  <header
    class="h-14 px-4 flex items-center justify-between bg-white dark:bg-[#1e293b] border-b border-gray-200 dark:border-gray-700 transition-colors duration-300 shrink-0 z-50"
  >
    <!-- Left: Logo & Nav -->
    <div class="flex items-center gap-8">
      <!-- Logo -->
      <div
        class="flex items-center gap-2 font-bold text-xl text-gray-900 dark:text-white cursor-pointer"
    ...
    <div class="flex items-center gap-3">
      ...
      <el-dropdown trigger="click" @command="handleThemeCommand">
        <button
          class="p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition-colors flex items-center justify-center"
        >
      ...
      <el-dropdown trigger="click" @command="handleLangCommand">
        <button
          class="flex items-center gap-1 px-2 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition-colors text-sm font-medium"
        >
          {{ locale === 'zh' ? '中文' : 'EN' }}
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="en" :class="{ 'text-blue-500': locale === 'en' }"
              >English</el-dropdown-item
            >
            <el-dropdown-item command="zh" :class="{ 'text-blue-500': locale === 'zh' }"
              >中文</el-dropdown-item
            >
          </el-dropdown-menu>
        </template>
