...
Stateful 模式提供会话持久化、SSE 流式传输和实时通知。无需添加额外请求头 — Claude Code 可通过 UA 匹配自动识别。
...
CherryStudio 可通过 UA 匹配自动识别并路由到 stateless 模式。Stateless 模式使用 per-request transport — 每次 POST 创建独立 transport，无会话持久化，GET 返回 405。
