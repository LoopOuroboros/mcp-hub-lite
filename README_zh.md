...
- **双模式会话**: 支持 stateful（会话持久化、SSE、通知推送）和 stateless（per-request）双模式，通过 UA 匹配 / 请求头 / 配置三级优先级切换
- **MCP 通知推送**: 推送 `notifications/tools/list_changed` 和 `notifications/resources/list_changed` 到连接的客户端
...
<details>
<summary>点击展开完整 <code>.mcp-hub.json</code> 配置示例</summary>
...
将 MCP 客户端连接到 MCP-HUB-LITE，在客户端 MCP 配置中添加以下内容：

<details>
<summary>Claude Code / Claude Desktop（stateful 模式 — 默认）</summary>
...
Stateful 模式提供会话持久化、SSE 流式传输和实时通知。UA 匹配非自动生效 — 需在 `.mcp-hub.json` 的 `system.session.sessionModeRules.stateful` 中手动添加 `"ClaudeCode"`。推荐直接使用默认 stateful 模式，Claude Code 等标准 MCP 客户端无需额外配置。

</details>

<details>
<summary>CherryStudio（stateless 模式）</summary>
...
CherryStudio 需要通过请求头 `x-mcp-session-mode: stateless` 来使用 stateless 模式。UA 匹配并非自动生效 — 需在 `.mcp-hub.json` 的 `system.session.sessionModeRules.stateless` 中手动添加 `"CherryStudio"`。推荐使用请求头方式，优先级最高。

Stateless 模式使用 per-request transport — 每次 POST 创建独立 transport，无会话持久化，GET 返回 405。

</details>

<details>
<summary>手动指定会话模式</summary>

添加 `x-mcp-session-mode` 请求头强制指定模式：
...
可选值：`"stateful"` 或 `"stateless"`。请求头优先级最高，覆盖 UA 匹配和配置默认值。
