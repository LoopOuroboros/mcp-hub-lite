...
### Q: 为什么使用复合键而不是简单的服务器 ID？

A: 复合键 (`serverName-serverIndex`) 支持多实例服务器架构，每个服务器可以有多个实例，每个实例需要独立的连接和状态管理。

### Q: 如何处理服务器删除事件？

A: ConnectionManager 监听 `SERVER_DELETED` 事件，自动断开该服务器的所有实例连接，确保资源正确清理。

### Q: SSE 传输为什么不刷新工具和资源？

A: SSE 是单向传输协议，服务器无法主动推送工具和资源列表。因此跳过这些操作以避免不必要的错误和性能开销。

### Q: 如何启用 MCP 通信调试日志？

A: 设置 `MCP_COMM_DEBUG=true` 环境变量：
...
1. **复合键级别**: 每个服务器实例的独立缓存
2. **服务器名称级别**: 聚合同一服务器所有实例的工具，用于搜索和发现场景
