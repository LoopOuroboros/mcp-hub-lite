...
### ConfigMigrator (`config-migrator.ts`)

**职责**: 配置版本迁移器，支持从 v1.0 自动迁移到 v1.1
...
- **ServerTemplate**: 服务器模板配置（共享基础配置）
- **ServerInstance**: 服务器实例配置（实例特定覆盖）
- **TagDefinitions**: 标签定义支持

**主要方法**:

- `migrateConfig(configPath, options)` - 执行配置迁移
- `dryRunMigration(configPath)` - 迁移干运行（不修改文件）
- `rollbackMigration(configPath, backupPath)` - 回滚迁移
- `checkMigrationStatus(configPath)` - 检查迁移状态
- `resolveInstanceConfig(serverConfig, instanceId)` - 解析实例配置
- `getEnabledInstances(serverConfig)` - 获取启用的实例
- `convertToV1ServerConfig(serverConfig, instanceId)` - 转换回 v1.0 格式
- `getFlatServersMap(config)` - 获取扁平化的 v1.0 格式服务器映射
...
interface MigrationOptions {
  ...
  validateAfterMigration?: boolean; // 迁移后验证
}
```
...
- 本文件现在仅作为向后兼容的重新导出点
- 所有核心 Schema 定义已移至 `shared/models/server.model.ts`
- 包含 v1.1 配置格式的完整支持
- 移除了 v1.0 兼容性代码
...
```typescript
// 从 @shared-models/server.model.ts 重新导出
export {
  TransportType,
  TagDefinitionSchema,
  ServerInstanceSchema,
  ServerInstanceUpdateSchema, // 新增：用于服务器实例部分更新
  ServerTemplateSchema,
  ServerConfigSchema,
  LoggingConfigSchema, // 更新：新增 apiDebug 字段
...
- 用于服务器实例的部分更新
- 不包含默认值，避免用空默认值覆盖现有配置
- 所有字段都是可选的

**LoggingConfigSchema 更新**:

- 新增 `apiDebug` 字段（默认 false）
- 用于启用 API 调试日志输出
...
| `aggregatedTools` | string[]                                        | `[]`      | 聚合的工具列表（v1.1，替代 allowedTools）       |
...
│   ├── depends on: config-migrator.ts (for auto-migration)
...
| `shared/models/server.model.ts`   | **核心 Schema 定义位置**（v1.1 格式） |
