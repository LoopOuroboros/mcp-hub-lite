...
├── name-converter.ts        # 名称规范化工具
├── composite-key.ts         # 复合键工具
├── instance-id.ts           # 实例 ID 工具
├── network-security.ts      # 网络安全工具
├── sort-utils.ts            # 排序工具
├── version.ts               # 版本信息
```
...
**职责**: Server-Sent Events 传输，由 SDK `SSEClientTransport` 直接在 `TransportFactory` 中实例化。
...
| `utils/composite-key.ts`                        | 复合键工具                    |
| `utils/instance-id.ts`                          | 实例 ID 工具                  |
| `utils/network-security.ts`                     | 网络安全工具                  |
| `utils/sort-utils.ts`                           | 排序工具                      |
| `utils/version.ts`                              | 版本信息                      |
