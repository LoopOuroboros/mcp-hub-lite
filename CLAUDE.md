...
graph TD
    ...
    SERVICES --> SEARCH["simple-search.service.ts"];
    ...
    click COMPONENTS "./frontend/src/components/CLCLAUDE.md" "查看 components 模块文档"
...
4. **模块化组织**
   - 核心层：`src/models/` - 数据模型
   - 服务层：`src/services/` - 核心服务组件（HubManager、SimpleSearch、StreamingGateway）
...
- **SimpleSearchService** (`src/services/simple-search.service.ts`): 直接遍历搜索实现，基于字符串匹配的模糊搜索
- **GatewayService** (`src/services/gateway.service.ts`): 基于HttpStream协议的MCP网关服务
...
完整的架构设计请参见：[`specs/001-develop/arch/architecture.md`](specs/001-develop/architecture.md)
