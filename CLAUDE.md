...
- **[工具搜索](src/services/hub-tools.service.ts)**: 跨所有服务器进行工具搜索和发现
- **[进程管理](src/pid/manager.ts)**: 支持通过 npx/uvx 启动和管理 MCP 服务器进程
- **[双模式会话](src/services/gateway/session-manager.ts)**: stateful（会话持久化、SSE、通知推送）和 stateless（per-request）双模式，通过 UA 匹配 / 请求头 / 配置三级优先级切换
- **[会话查询 API](src/api/web/sessions.ts)**: `GET /web/sessions` 端点查询所有活跃 MCP 会话信息
- **MCP 通知推送**: 向连接的客户端推送 `notifications/tools/list_changed` 和 `notifications/resources/list_changed`
...
- CLI入口：`src/cli/index.ts` - 处理命令行参数
...
- **GatewayService**: MCP网关服务（HTTP-Stream传输、通知推送）
- **SessionManager**: 网关会话管理器（stateful/stateless 双模式、SSE 引用计数、SDK PING 存活探测、3 秒去抖通知广播）
- **McpConnectionManager**: 服务器连接和工具调用（`src/services/connection/connection-manager.ts`）
- **SessionManager**: 网关会话管理器（stateful/stateless 双模式、SSE 引用计数、SDK PING 存活探测）
- **HubToolsService**: 工具搜索、系统工具定义和服务聚合（`src/services/hub-tools.service.ts`）
...
1. 请求头 `x-mcp-session-mode: stateful | stateless`
2. UA 关键词匹配（`system.session.sessionModeRules`）
3. 配置默认值 `system.session.defaultSessionMode`（默认 `stateful`）

| 模式          | 适用场景                      | 行为                                                             |
| ------------- | ----------------------------- | ---------------------------------------------------------------- |
| **stateful**  | Claude Code 等标准 MCP 客户端 | 会话持久化、SSE 长连接、通知推送、SDK PING 存活探测              |
| **stateless** | CherryStudio 等客户端         | 每次 POST 创建 per-request transport，无会话持久化，GET 返回 405 |

**功能特性**:

- 纯内存会话状态管理（无磁盘 I/O）
- 基于 sessionId 的会话隔离
- SSE 引用计数 — 防止活跃 SSE 流被意外清理
- SDK PING 存活探测 — 30s 冷却 / 10s 超时，探活失败则标记会话失效
- 3 秒去抖通知广播 — `notifications/tools/list_changed` 和 `notifications/resources/list_changed`
- 可配置空闲超时（`security.idleConnectionTimeout`，默认 5 分钟）

**相关文件**:

- `src/services/gateway/session-manager.ts` - 网关会话管理器
- `src/services/gateway/gateway.service.ts` - 通知推送初始化
- `src/api/web/sessions.ts` - 会话查询 API
- `shared/models/constants.ts` - 会话模式常量
...
- **Stateful 模式**: 每个客户端连接获得专属的 `StreamableHTTPServerTransport` + `McpServer` 实例对，SDK 负责会话 ID 生成与生命周期管理
- **Stateless 模式**: 每个 POST 请求创建独立的 per-request transport，请求完成后即可回收，无会话持久化
...
| `mcp-hub-lite status`             | 查看服务状态（含活跃会话列表） |
...
```json
{
  "system": {
    "session": {
      ...
      "sessionModeRules": {
        "stateful": ["Claude", "ClaudeCode"],
        "stateless": ["CherryStudio", "OpenWebUI"]
      }
    }
  }
}
...
- `defaultSessionMode`: 默认会话模式，`stateful` 或 `stateless`
- `sessionModeRules.stateful`: UA 关键词列表，匹配时使用 stateful 模式
- `sessionModeRules.stateless`: UA 关键词列表，匹配时使用 stateless 模式
