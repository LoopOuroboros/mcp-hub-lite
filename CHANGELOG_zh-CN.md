...
- 移除构造函数中的急切工具缓存重建调用，消除 CLI start/stop 命令执行时的多余日志输出
...
- search_tools 新增 token 化多分隔符搜索，支持命中数排序和每服务器可配置结果数量限制

### 文档

- 明确 stateless 模式需通过请求头指定，UA 匹配需手动配置
...
- 新增 search_tools 系统工具，支持跨服务器工具搜索
- /web/search 端点按各服务器 aggregatedTools 配置过滤聚合工具

### 前端

- 修复聚合工具网关页因 API 响应格式不匹配导致无法显示工具的问题
- 服务器工具页新增批量全聚合/零聚合按钮
- 布尔和数字标签值使用专用输入控件约束
...
- 修复 SSE 超时导致并发请求计数器漂移，引发新客户端被 503 拒绝的问题
- 新增 GET /health/connections 诊断端点用于查看连接统计

## [1.2.7] - 2026-05-09

### 网关

- 为 Streamable HTTP 服务器添加 OAuth 浏览器认证
- 完善 SERVER*INSTANCE*\* 事件管道，支持细粒度本地 WebSocket 更新

### 安全

- 运行时强制执行允许的网络、连接限制和超时配置

### 核心

- 开发模式下端口被占用时自动递增
- 改进 MCP 服务器启动失败日志的可见性
- 移除死代码和未使用的类型
...
- 使用运行时连接状态进行实例选择
- 澄清实例过滤注释
- 修复 listMcpResources 返回不完整服务器资源的问题，使用两遍遍历检查所有实例连接状态

### 工具

- 为 normalizeToolName 添加空值检查
...
- 增强 hub-tools 系统，改进工具处理
- 添加 list_tags 工具常量和更新 CLAUDE.md 文档
- 添加全面的使用指南文档
- 改进默认服务器描述文本
- 移除 listResources 中的陈旧缓存
- 澄清实例过滤注释
- 修复 listMcpResources 返回不完整服务器资源的问题，使用两遍遍历检查所有实例连接状态
- 使资源生成逻辑与 /web/mcp/status API 行为保持一致
...
- 在中文 README 中添加"解决痛点"章节
- 在中文 README 中添加 LinuxDo 话题链接
...
- 添加 use-guide 命令用于 CLI 使用参考
- 添加 install 命令用于添加新的 MCP 服务器
- 移除冗余的 npm scripts

### 网关

- 使用 gatewayDebug 条件包装初始化处理程序调试日志
